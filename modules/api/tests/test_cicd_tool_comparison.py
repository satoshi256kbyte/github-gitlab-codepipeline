...
class TestCICDToolEndpointComparison:
    ...
    ENDPOINTS = {
        "github": {
            ...
            "lambda_url": "https://github-local-api.execute-api.us-east-1.amazonaws.com/prod"  # 実際のAPI Gateway URLに置き換え
        },
        "gitlab": {
            "name": "GitLab CI/CD", 
            "port": 8081,
            ...
            "lambda_url": "https://gitlab-local-api.execute-api.us-east-1.amazonaws.com/prod"  # 実際のAPI Gateway URLに置き換え
        },
        "codepipeline": {
            "name": "CodePipeline",
            "port": 8082,
            ...
            "lambda_url": "https://codepipeline-local-api.execute-api.us-east-1.amazonaws.com/prod"  # 実際のAPI Gateway URLに置き換え
        }
    }
    ...
    def _make_request(self, url: str, method: str = "GET", json_data: Optional[Dict] = None, timeout: int = 30) -> Optional[requests.Response]:
        """
        ...
        for target in deployment_targets:
            if target in tool_config:
                ...
                if response is not None:
                    assert response.status_code == 200, f"{tool_config['name']} {target} ヘルスチェックが失敗しました"
                    
                    health_data = response.json()
                    assert health_data["status"] == "healthy", f"{tool_config['name']} {target} のステータスが正常ではありません"
                    assert "timestamp" in health_data, f"{tool_config['name']} {target} のレスポンスにタイムスタンプがありません"
                else:
                    pytest.skip(f"{tool_config['name']} {target} エンドポイントにアクセスできません（デプロイされていない可能性があります）")
    ...
    def test_version_endpoints(self, endpoint_configs, tool_name):
        ...
        for target in deployment_targets:
            if target in tool_config:
                ...
                if response is not None:
                    assert response.status_code == 200, f"{tool_config['name']} {target} バージョン情報取得が失敗しました"
                    
                    version_data = response.json()
                    assert "version" in version_data, f"{tool_config['name']} {target} のレスポンスにバージョン情報がありません"
                    assert version_data["version"] == "1.0.0", f"{tool_config['name']} {target} のバージョンが期待値と異なります"
                    assert "build_time" in version_data, f"{tool_config['name']} {target} のレスポンスにビルド時間がありません"
                else:
                    pytest.skip(f"{tool_config['name']} {target} エンドポイントにアクセスできません（デプロイされていない可能性があります）")
    ...
    def test_basic_crud_operations(self, endpoint_configs, tool_name):
        ...
        for target in deployment_targets:
            if target in tool_config:
                ...
                assert list_response.status_code == 200, f"{tool_config['name']} {target} アイテム一覧取得が失敗しました"
                ...
                create_response = self._make_request(f"{base_url}/api/items", "POST", test_item)
                assert create_response.status_code == 201, f"{tool_config['name']} {target} アイテム作成が失敗しました"
                ...
                assert get_response.status_code == 200, f"{tool_config['name']} {target} アイテム個別取得が失敗しました"
                
                retrieved_item = get_response.json()
                assert retrieved_item["name"] == test_item["name"], f"{tool_config['name']} {target} 取得したアイテム名が一致しません"
                assert retrieved_item["description"] == test_item["description"], f"{tool_config['name']} {target} 取得したアイテム説明が一致しません"
                ...
                update_response = self._make_request(f"{base_url}/api/items/{item_id}", "PUT", update_data)
                assert update_response.status_code == 200, f"{tool_config['name']} {target} アイテム更新が失敗しました"
                
                updated_item = update_response.json()
                assert updated_item["name"] == update_data["name"], f"{tool_config['name']} {target} 更新されたアイテム名が一致しません"
                
                # 5. アイテム削除
                delete_response = self._make_request(f"{base_url}/api/items/{item_id}", "DELETE")
                assert delete_response.status_code == 204, f"{tool_config['name']} {target} アイテム削除が失敗しました"
                
                # 6. 削除確認
                get_deleted_response = self._make_request(f"{base_url}/api/items/{item_id}")
                assert get_deleted_response.status_code == 404, f"{tool_config['name']} {target} 削除されたアイテムが取得できてしまいました"
                
                # 7. 最終的なアイテム数確認
                final_list_response = self._make_request(f"{base_url}/api/items")
                assert final_list_response.status_code == 200, f"{tool_config['name']} {target} 最終アイテム一覧取得が失敗しました"
                
                final_data = final_list_response.json()
                assert final_data["total"] == initial_count, f"{tool_config['name']} {target} 最終的なアイテム数が初期状態と一致しません"
        ...
        for tool_name, item_info in created_items.items():
            ...
            for other_tool_name, other_tool_config in endpoint_configs.items():
                if tool_name != other_tool_name:
                    ...
                    get_response = self._make_request(f"{other_base_url}/api/items/{item_id}")
                    
                    if get_response is not None:
                        # 404が返されるか、異なるアイテムが返されることを確認
                        assert get_response.status_code == 404 or (
                            get_response.status_code == 200 and 
                            get_response.json()["name"] != item_info["item"]["name"]
        ...
        if len(response_formats) > 1:
            ...
            for tool_name, format_info in response_formats.items():
                if tool_name != first_tool:
                    ...
                    assert format_info["health_status"] == first_format["health_status"], \
...
class TestCICDToolPerformanceComparison:
    ...
    def test_endpoint_response_time(self, tool_name):
        ...
        try:
            ...
            # 基本的な応答性チェック（30秒以内）
            assert response_time < 30.0, f"{tool_config['name']}のレスポンス時間が30秒を超えました"
            assert response.status_code == 200, f"{tool_config['name']}のヘルスチェックが失敗しました"
            
        except requests.exceptions.RequestException:
            pytest.skip(f"{tool_config['name']} エンドポイントにアクセスできません（デプロイされていない可能性があります）")