...
class TestCICDToolIsolation:
    ...
    async def test_concurrent_health_checks(self):
        ...
        for result in results:
            assert result["success"], f"{result['tool']} endpoint failed: {result.get('error', 'Unknown error')}"
    ...
    async def test_concurrent_crud_operations(self):
            ...
            try:
                async with httpx.AsyncClient(timeout=30.0) as client:
                    ...
                    if create_response.status_code != 201:
                        return {"tool": tool_name, "success": False, "error": "Create failed"}
                    ...
                    if get_response.status_code != 200:
                        return {"tool": tool_name, "success": False, "error": "Get failed"}
                    
                    item_data = get_response.json()
                    
                    # データの整合性確認
                    if item_data["name"] != create_data["name"]:
                        return {"tool": tool_name, "success": False, "error": "Data mismatch"}
        ...
        # 全ての操作が成功することを確認
        for result in results:
            assert result["success"], f"{result['tool']} CRUD operations failed: {result.get('error', 'Unknown error')}"
        
        # 各ツールで作成されたアイテムが正しく分離されていることを確認
        item_names = [r["item_name"] for r in results if r["success"]]
        assert len(set(item_names)) == len(item_names), "Item names should be unique across tools"
        
        for result in results:
            expected_name = f"{result['tool']}_test_item"
            assert result["item_name"] == expected_name, f"Item name mismatch for {result['tool']}"
    ...
    async def test_load_balancer_port_isolation(self):
        ...
        if accessible_count > 0:
            ...
            pytest.skip("CI/CD tool endpoints not accessible - infrastructure not deployed")