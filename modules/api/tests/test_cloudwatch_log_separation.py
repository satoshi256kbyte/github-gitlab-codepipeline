...
class TestCloudWatchLogSeparation:
    ...
    def test_log_groups_exist(self):
        ...
        try:
            ...
            if missing_groups:
                ...
                # 全て存在しない場合はスキップ
                if len(missing_groups) == len([lg for services in self.log_group_patterns.values() for lg in services.values()]):
                    pytest.skip("No CI/CD tool log groups found - infrastructure not deployed")
    ...
    def test_log_group_naming_convention(self):
        ...
        try:
            ...
            cicd_log_groups = [
                lg for lg in existing_log_groups 
                if any(tool in lg for tool in ['github-local', 'gitlab-local', 'codepipeline-local'])
            ]
                ...
                for service, log_group_name in services.items():
                    if log_group_name in existing_log_groups:
                        try:
                            ...
                            stream_names = [stream['logStreamName'] for stream in streams_response['logStreams']]
                            tool_log_streams[tool][service] = stream_names
                            
                            print(f"✓ {tool} {service} has {len(stream_names)} log streams")
                            
                        except Exception as e:
                            print(f"⚠ Cannot access log streams for {log_group_name}: {str(e)}")
    ...
    def test_recent_log_entries_separation(self):
        ...
        try:
            ...
            for tool, services in self.log_group_patterns.items():
                ...
                for service, log_group_name in services.items():
                    try:
                        ...
                        if events:
                            print(f"✓ {tool} {service}: {len(events)} log entries in last hour")
                            ...
                            for message in sample_messages:
                                ...
                                # （実際のログ内容は実装依存）
                                assert isinstance(message, str), "Log message should be string"
            ...
            if total_entries == 0:
                pytest.skip("No recent log entries found - applications may not be active")
            
            print(f"Total log entries found: {total_entries}")
            
            # 各ツールのログが独立して記録されていることを確認
            tools_with_logs = [
                tool for tool, services in log_entries_by_tool.items()
