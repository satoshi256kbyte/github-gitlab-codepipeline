...
class TestErrorHandling:
    ...
    def test_422_validation_error(self, client: TestClient, clean_items_storage):
        ...
        assert "description" in data["message"]  # 必須フィールドのエラーメッセージ

        # descriptionフィールドのエラーが含まれていることを確認（メッセージ内で確認）
        assert "Field required" in data["message"]

    @pytest.mark.unit
    def test_validation_error_empty_fields(
        self, client: TestClient, clean_items_storage
    ):
    ...
    def test_missing_content_type(self, client: TestClient):
        ...
        valid_data = {"name": "テストアイテム", "description": "テスト用の説明"}
    ...
    @pytest.mark.unit
    def test_update_item_not_found_error(self, client: TestClient, clean_items_storage):
        """存在しないアイテムの更新エラーテスト"""
        update_data = {"name": "更新されたアイテム", "description": "更新された説明"}
    ...
    @pytest.mark.unit
    def test_delete_item_not_found_error(self, client: TestClient, clean_items_storage):
        """存在しないアイテムの削除エラーテスト"""
        non_existent_id = 999
    ...
    def test_invalid_item_id_type(self, client: TestClient):
        ...
        # 型エラーが含まれていることを確認
        type_errors = [
            error for error in errors if "type" in error and "int" in str(error)
        ]
