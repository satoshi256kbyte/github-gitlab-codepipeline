...
'''
    ...
    def _test_pipeline_failure(self, tool: str, failure_type: str, temp_dir: str):
        ...
        # パイプラインが適切に失敗したことを確認
        assert not result["success"], f"{tool} パイプラインが {failure_type} エラーで失敗しませんでした"
        assert failure_type in result.get("failure_stage", ""), f"{tool} パイプラインが期待されたステージ（{failure_type}）で失敗しませんでした"
...
class TestPipelineFailureRecovery:
    ...
    def test_failure_notification_mechanisms(self):
        ...
        for tool in tools:
            ...
            # 通知が適切に設定されていることを確認
            assert notification_config["configured"], f"{tool} の失敗通知が設定されていません"
    ...
    def test_rollback_mechanisms(self):
        ...
        for tool in tools:
            for target in deployment_targets:
                ...
                assert rollback_config["available"], f"{tool} {target} のロールバック機能が利用できません"
    ...
    def test_failure_detection_speed(self):
        ...
        for tool in tools:
            ...
            for failure_type in failure_types:
                ...
                # 失敗が適切に検出されることを確認
                assert detection_result["detected"], f"{tool} {failure_type} 失敗が検出されませんでした"
                
                print(f"{tool.upper()} {failure_type} 失敗検出時間: {detection_time:.2f}秒")
