...
class CICDConfigAnalyzer:
    ...
    def analyze_yaml_structure(self, content: str) -> Dict[str, Any]:
        ...
        try:
            ...
            def max_depth(obj, current_depth=0):
                """最大ネスト深度を計算"""
                if isinstance(obj, dict):
                    return max([max_depth(v, current_depth + 1) for v in obj.values()] + [current_depth])
                elif isinstance(obj, list):
                    return max([max_depth(item, current_depth + 1) for item in obj] + [current_depth])
                else:
    ...
    def analyze_github_actions(self, content: str, yaml_data: Dict) -> Dict[str, Any]:
        ...
        if "jobs" in yaml_data:
            ...
            analysis["parallel_jobs"] = len(yaml_data["jobs"])  # GitHub Actionsは基本的に並列実行
    ...
    def analyze_gitlab_ci(self, content: str, yaml_data: Dict) -> Dict[str, Any]:
        ...
        # ジョブの分析
        jobs = {k: v for k, v in yaml_data.items() 
                if isinstance(v, dict) and k not in ["stages", "variables", "cache", "before_script", "after_script"]}
    ...
    def analyze_codepipeline_buildspecs(self) -> Dict[str, Any]:
        ...
        buildspec_files = list(self.buildspec_dir.glob("*.yml")) + list(self.buildspec_dir.glob("*.yaml"))
    ...
    def calculate_complexity_score(self, metrics: Dict[str, Any], tool_specific: Dict[str, Any]) -> float:
        """複雑さスコアを計算"""
        # 基本メトリクス（重み付き）
        base_score = (
    ...
    def analyze_all_configs(self) -> List[ConfigAnalysis]:
        ...
        if gh_file.exists():
            ...
            if yaml_analysis["valid_yaml"]:
                gh_specific = self.analyze_github_actions(metrics["content"], yaml_analysis["yaml_data"])
                
                complexity = self.calculate_complexity_score(
                    {**metrics, **yaml_analysis}, gh_specific
                )
                
                results.append(ConfigAnalysis(
        ...
        if gl_file.exists():
            ...
            if yaml_analysis["valid_yaml"]:
                gl_specific = self.analyze_gitlab_ci(metrics["content"], yaml_analysis["yaml_data"])
    ...
    def generate_comparison_report(self, analyses: List[ConfigAnalysis]) -> str:
        ...
        report.append("")
        report.append(f"分析日時: {__import__('datetime').datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        report.append("")
        
        # サマリーテーブル
        report.append("## 概要比較")
        report.append("")
        report.append("| ツール | ファイル数 | コード行数 | ジョブ数 | ステップ数 | 並列ジョブ数 | 複雑さスコア |")
        report.append("|--------|------------|------------|----------|------------|--------------|--------------|")
        
        for analysis in analyses:
            file_count = 1 if analysis.tool_name != "CodePipeline" else len(list(self.buildspec_dir.glob("*.yml")))
            report.append(f"| {analysis.tool_name} | {file_count} | {analysis.line_count} | {analysis.job_count} | {analysis.step_count} | {analysis.parallel_jobs} | {analysis.complexity_score:.1f} |")
        ...
        for analysis in analyses:
            ...
            report.append(f"- **キャッシュ使用**: {'✓' if analysis.cache_usage else '✗'}")
            report.append(f"- **複雑さスコア**: {analysis.complexity_score:.1f}")
            report.append(f"- **機能**: {', '.join(analysis.features) if analysis.features else 'なし'}")
            report.append("")
        ...
        report.append(f"- **最も複雑**: {most_complex.tool_name} (スコア: {most_complex.complexity_score:.1f})")
        report.append(f"- **最もシンプル**: {least_complex.tool_name} (スコア: {least_complex.complexity_score:.1f})")
        report.append("")
        ...
        report.append(f"- **最も記述量が多い**: {max_lines.tool_name} ({max_lines.line_count}行)")
        report.append(f"- **最も記述量が少ない**: {min_lines.tool_name} ({min_lines.line_count}行)")
        report.append("")
        
        # 並列度比較
        max_parallel = max(analyses, key=lambda x: x.parallel_jobs)
        
        report.append(f"- **最も並列度が高い**: {max_parallel.tool_name} ({max_parallel.parallel_jobs}並列ジョブ)")
        report.append("")
        ...
        if all_features:
            report.append("### 機能比較")
            report.append("")
            report.append("| 機能 | " + " | ".join([a.tool_name for a in analyses]) + " |")
    ...
    def save_analysis_results(self, analyses: List[ConfigAnalysis], output_dir: str = "docs"):
        """分析結果をファイルに保存"""
...
def main():
    ...
    parser.add_argument("--format", choices=["console", "file", "both"], default="both", help="出力形式")
