...
class CICDPerformanceAnalyzer:
    ...
    def __init__(self):
        self.results = {"github": {}, "gitlab": {}, "codepipeline": {}}
        self.start_times = {}
        self.endpoints = {
            "github": {
                "name": "GitHub Actions",
                "lambda_url": "https://github-local-api.execute-api.ap-northeast-1.amazonaws.com/prod",
                "alb_url": "http://github-local-alb-api.example.com",
                "port": 8080,
            },
            "gitlab": {
                "name": "GitLab CI/CD",
                "lambda_url": "https://gitlab-local-api.execute-api.ap-northeast-1.amazonaws.com/prod",
                "alb_url": "http://gitlab-local-alb-api.example.com",
                "port": 8081,
            },
            "codepipeline": {
                "name": "CodePipeline",
                "lambda_url": "https://codepipeline-local-api.execute-api.ap-northeast-1.amazonaws.com/prod",
    ...
    def _measure_github_actions(self) -> dict:
        ...
        try:
            ...
            return {
                "success": latest_run["status"] == "completed"
    ...
    def _measure_gitlab_cicd(self) -> dict:
        ...
        try:
            ...
            if not gitlab_token or not gitlab_project_id:
                return {
                    "success": False,
                    "error": "GitLab token or project ID not configured",
                }
            ...
            response = requests.get(
                pipelines_url, headers=headers, params={"per_page": 1}
            )

            if response.status_code != 200:
                return {
                    "success": False,
                    "error": f"Failed to fetch pipelines: {response.status_code}",
                }
            ...
            if jobs_response.status_code != 200:
                return {
                    "success": False,
                    "error": f"Failed to fetch jobs: {jobs_response.status_code}",
                }
    ...
    def _measure_codepipeline(self) -> dict:
        ...
        try:
            ...
            json.loads(detail_result.stdout)
            ...
            for action in action_executions:
                ...
                if action.get("startTime") and action.get("lastUpdateTime"):
                    start = datetime.fromisoformat(
                        action["startTime"].replace("Z", "+00:00")
                    )
                    end = datetime.fromisoformat(
                        action["lastUpdateTime"].replace("Z", "+00:00")
                    )
                    duration = (end - start).total_seconds()
    ...
    def _simulate_deployment(self, tool: str, target: str) -> dict:
        ...
        return {"success": True, "target": target, "tool": tool, "simulated": True}
    ...
    def generate_comparison_report(self, results: list[dict]) -> dict:
        ...
        if execution_times:
            ...
            report["comparison_summary"]["execution_time"] = {
                ...
                "time_difference_seconds": execution_times[slowest_tool]
                - execution_times[fastest_tool],
            }
...
def main():
    ...
    parser.add_argument(
        "--measure-deployment", action="store_true", help="デプロイ速度も測定する"
    )
    ...
    if "execution_time" in comparison_report["comparison_summary"]:
        ...
        print(
            f"最速ツール: {exec_summary['fastest']['tool']} ({exec_summary['fastest']['time_seconds']:.2f}秒)"
        )
        print(
            f"最遅ツール: {exec_summary['slowest']['tool']} ({exec_summary['slowest']['time_seconds']:.2f}秒)"
        )
