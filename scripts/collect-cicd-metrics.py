...
class CICDMetricsCollector:
    ...
    def __init__(self):
        ...
        # AWS クライアント初期化
        self.codepipeline_client = boto3.client("codepipeline", region_name=self.aws_region)
    ...
    def collect_github_metrics(self, repo_owner: str, repo_name: str, days: int = 7) -> List[PipelineMetrics]:
        """
        ...
        try:
            ...
            for run in runs_data.get("workflow_runs", []):
                # 各実行の詳細メトリクスを取得
                run_metrics = self._extract_github_run_metrics(run, headers, repo_owner, repo_name)
                if run_metrics:
    ...
    def _extract_github_run_metrics(self, run: Dict, headers: Dict, repo_owner: str, repo_name: str) -> Optional[PipelineMetrics]:
    ...
    def collect_gitlab_metrics(self, days: int = 7) -> List[PipelineMetrics]:
        ...
        try:
            ...
            for pipeline in pipelines:
                # 各パイプラインの詳細メトリクスを取得
                pipeline_metrics = self._extract_gitlab_pipeline_metrics(pipeline, headers)
                if pipeline_metrics:
    ...
    def _extract_gitlab_pipeline_metrics(self, pipeline: Dict, headers: Dict) -> Optional[PipelineMetrics]:
        ...
        try:
            ...
            if pipeline.get("created_at") and pipeline.get("updated_at"):
                start_time = datetime.fromisoformat(pipeline["created_at"].replace("Z", "+00:00"))
                end_time = datetime.fromisoformat(pipeline["updated_at"].replace("Z", "+00:00"))
                total_duration = (end_time - start_time).total_seconds()
            else:
                total_duration = sum(stage["duration_seconds"] for stage in stages)
    ...
    def collect_codepipeline_metrics(self, pipeline_name: str, days: int = 7) -> List[PipelineMetrics]:
        """
    ...
    def _extract_codepipeline_execution_metrics(self, pipeline_name: str, execution: Dict) -> Optional[PipelineMetrics]:
        ...
        try:
            ...
            for action in actions:
                if action.get("startTime") and action.get("lastUpdateTime"):
                    duration = (action["lastUpdateTime"] - action["startTime"]).total_seconds()
            ...
            if execution.get("startTime") and execution.get("lastUpdateTime"):
                total_duration = (execution["lastUpdateTime"] - execution["startTime"]).total_seconds()
            else:
                total_duration = sum(stage["duration_seconds"] for stage in stages)
            ...
            return PipelineMetrics(
                ...
                end_time=execution.get("lastUpdateTime", execution["startTime"]).isoformat(),
                duration_seconds=total_duration,
    ...
    def collect_deployment_metrics(self, tool: str, days: int = 7) -> List[DeploymentMetrics]:
        """
        デプロイメントメトリクスを収集
    ...
    def _collect_target_deployment_metrics(self, tool: str, target: str, days: int) -> List[DeploymentMetrics]:
    ...
    def _collect_lambda_deployment_metrics(self, tool: str, days: int) -> List[DeploymentMetrics]:
        """Lambda デプロイメントメトリクスを収集"""
        # CloudWatch Logs から SAM デプロイメントログを取得
        # 実際の実装では、CloudWatch Logs API を使用してデプロイログを解析
        return []
    
    def _collect_ecs_deployment_metrics(self, tool: str, days: int) -> List[DeploymentMetrics]:
        """ECS デプロイメントメトリクスを収集"""
        # ECS サービスのデプロイメント履歴を取得
        # 実際の実装では、ECS API を使用してデプロイメント履歴を取得
        return []
    
    def _collect_ec2_deployment_metrics(self, tool: str, days: int) -> List[DeploymentMetrics]:
        """EC2 デプロイメントメトリクスを収集"""
        # CodeDeploy のデプロイメント履歴を取得
        # 実際の実装では、CodeDeploy API を使用してデプロイメント履歴を取得
        return []
    
    def generate_metrics_report(self, pipeline_metrics: Dict[str, List[PipelineMetrics]], 
                              deployment_metrics: Dict[str, List[DeploymentMetrics]]) -> Dict:
        """
        メトリクスレポートを生成
        ...
        for tool, metrics in deployment_metrics.items():
            if metrics:
                avg_duration = sum(m.duration_seconds for m in metrics) / len(metrics)
                rollback_rate = sum(1 for m in metrics if m.rollback_occurred) / len(metrics)
                
                report["deployment_analysis"][tool] = {
                    "total_deployments": len(metrics),
                    "average_duration_seconds": avg_duration,
                    "rollback_rate": rollback_rate,
                    "targets": list(set(m.target for m in metrics))
                }
        ...
        if len(pipeline_metrics) > 1:
            ...
            for tool in tools:
                if pipeline_metrics[tool]:
                    durations[tool] = sum(m.duration_seconds for m in pipeline_metrics[tool]) / len(pipeline_metrics[tool])
            ...
            if durations:
                ...
                report["comparison"]["execution_time"] = {
                    ...
                    "time_difference_seconds": durations[slowest_tool] - durations[fastest_tool],
                    "performance_improvement_percentage": 
                        ((durations[slowest_tool] - durations[fastest_tool]) / durations[slowest_tool]) * 100
                }
...
def main():
    ...
    parser.add_argument("--codepipeline-name", type=str, default="cicd-comparison-pipeline",
                       help="CodePipeline名")
    parser.add_argument("--include-deployments", action="store_true",
    ...
    for tool in args.tools:
        ...
        if tool == "github":
            if args.github_repo:
                ...
                pipeline_metrics[tool] = collector.collect_github_metrics(owner, repo, args.days)
            else:
                print("GitHub リポジトリが指定されていません (--github-repo owner/repo)")
            ...
            pipeline_metrics[tool] = collector.collect_codepipeline_metrics(args.codepipeline_name, args.days)
        
        # デプロイメントメトリクス収集（オプション）
        if args.include_deployments:
            deployment_metrics[tool] = collector.collect_deployment_metrics(tool, args.days)
        else:
    ...
    if "comparison" in report and "execution_time" in report["comparison"]:
        ...
        print(f"  パフォーマンス改善率: {comparison['performance_improvement_percentage']:.1f}%")
